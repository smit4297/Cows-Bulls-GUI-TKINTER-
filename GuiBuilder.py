...
class BullsGui(object):
    ...
    def __init__(self, master):
        ...
        cur_column += 1
        self.rules = "==> Secret code of will be generated by computer.\n" \
        ...
        cur_row += 1
        self.number_of_digits = 4   #by default set to 4
        self.digits_input = tk.Label(master, text="Enter Number Of Digits", fg='#9F1AAC', font=self.text_font)
        self.digits_input.configure(anchor="center")
        self.digits_input.grid(row=cur_row, columnspan=10, sticky='ew')

        cur_row += 1
        vcmd_digit = (master.register(self.on_validate_digit), '%P', '%S')
        self.digits = tk.Entry(master,
                               validate='key',
                               justify='center',
                               validatecommand=vcmd_digit)
        self.digits.grid(row=cur_row, columnspan=10, sticky='ew')

        cur_row += 1

        self.okay_button = tk.Button(master, text='Okay', )
        self.okay_button.columnconfigure(cur_row, weight=0)
        self.okay_button.grid(row=cur_row, columnspan=10, sticky='ew')

        cur_row += 2
        self.score_label = tk.Label(master, text='score: 1', font=self.title_font, fg='#f00')
        self.score_label.configure(anchor="center")
        self.score_label.grid(row=cur_row, columnspan=10, sticky='ew')
        ...
        self.game_result = tk.Label(master, text='', fg='#f00', font=self.text_font)
    ...
    def on_validate(self, P, S):
        # self.game_result.config(text = f"{P} - {S}")
        '''
        Only digits can be entered and the string can't
        be longer than length of digits
        '''
        if S.isdigit() and len(P) < self.number_of_digits + 1:
            return True
        else:
            self.master.bell()
            return False

    def on_validate_digit(self, P, S):
        # self.game_result.config(text = f"{P} - {S}")
        '''
        Only digits can be entered and the string can't
        be longer than 2
        '''
        if S.isdigit() and len(P) < 3:
            return True
        else:
            self.master.bell()
            return False
    ...
    def update_bulls_cows(self, player_counts):
        '''
        Updates the count labels according to the actual score
        '''
        self.player_bull_count.config(text=str(player_counts[1]))
        self.player_cow_count.config(text=str(player_counts[0]))

    def update_score(self, score_number):
        '''
        Updates the score counter
        '''
        self.score_label.config(text='score: {0}'.format(score_number))
